...
export class SelectComponent implements ControlValueAccessor {
  ...
  get selectClasses(): string {
    const classes = ['x86-select__trigger'];
    
    classes.push(`x86-select__trigger--${this.size}`);
    
    if (this.error) classes.push('x86-select__trigger--error');
    if (this.disabled) classes.push('x86-select__trigger--disabled');
    
    return classes.join(' ');
  }

  get wrapperClasses(): string {
    const classes = ['x86-select'];
    
    if (this.error) classes.push('x86-select--error');
    if (this.disabled) classes.push('x86-select--disabled');
