...
export class ProfileComponent implements OnInit {
  ...
  private loadUserGrids(userId: string): void {
    ...
    this.supabaseService.getUserGrids(userId)
      .pipe(takeUntilDestroyed())
      .subscribe({
        next: (grids) => {
  ...
  getTotalShares(): number {
    return this.userGrids().reduce((total, grid) => total + grid.socialMetrics.shares, 0);
  }
  ...
  editProfile(): void {
    ...
    console.log('Edit profile functionality will open a modal or navigate to edit page');
  ...
  async monetizeGrid(gridId: string): Promise<void> {
    try {
      ...
      const updatedGrids = this.userGrids().map(grid => 
        grid.id === gridId 
          ? { ...grid, status: 'monetized' as const }
          : grid
      );
